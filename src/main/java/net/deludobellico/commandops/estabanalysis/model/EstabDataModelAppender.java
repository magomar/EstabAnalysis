 ...
 */
public class EstabDataModelAppender implements EstabDataModelBuilder{
    ...
    public EstabDataModelAppender(String estabName, List<EstabData> estabDataList) {
...
//        e.getImages().stream().forEach(m -> track(m, images));
//        e.getSides().stream().forEach(m -> track(m, sides));
//        e.getNations().stream().forEach(m -> track(m, nations));
//        e.getServices().stream().forEach(m -> track(m, services));
//        e.getForces().stream().forEach(m -> track(m, forces));
//        e.getVehicles().stream().forEach(m -> track(m, vehicles));
//        e.getWeapons().stream().forEach(m -> track(m, weapons));
//        e.getAmmos().stream().forEach(m -> track(m, ammos));
//        e.getRadios().stream().forEach(m -> track(m, radios));
//        e.getFormationEffects().stream().forEach(m -> track(m, formationEffects));
